...
void main() {
  ...
  group('Acceptance: Feature flags UI + Budget enforcement gating', () {
    ...
    testWidgets('Feature flags toggles persist via GetStorage', (tester) async {
      // Ensure required services are registered
      await _ensureTestServices();
      // Start with the page
      await tester.pumpWidget(const MaterialApp(home: PerformanceFlagsPage()));
      ...
      expect(FeatureFlags.instance.virtualizationTuningEnabled, equals(virtAfter));
      expect(FeatureFlags.instance.fixedExtentListEnabled, equals(fixedAfter));
    }, timeout: const Timeout(Duration(minutes: 2)));

    testWidgets('Cache budget enforcement evicts to soft max (gated)', (tester) async {
      ...
      try {
        ...
        // Initialize services
        final budget = MemoryBudgetService.instance; // onInit reads perf config
        ...
        expect(postBytes <= softMax + wiggle, true,
            reason: 'Cache should be trimmed to soft max. post=$postBytes, softMax=$softMax');
      } finally {
