...
void main() {
  ...
  testWidgets('Perf: open inbox demo and long continuous scroll', (tester) async {
    ...
    final controller = PrimaryScrollController.of(tester.element(scrollFinder));
    for (int i = 0; i < 20; i++) {
    ...
    try {
      ...
      if (file.existsSync()) {
        ...
        frameBudgetMs = (cfg['frame_budget_ms'] as num?)?.toDouble() ?? frameBudgetMs;
        p95JankyPctMax = (cfg['p95_janky_frames_pct_max'] as num?)?.toDouble() ?? p95JankyPctMax;
      }
    } catch (_) {}
