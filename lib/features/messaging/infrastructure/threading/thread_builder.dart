...
class ThreadBuilder {
  ...
  Future<List<ThreadAggregate>> build({required String folderId, String? requestId}) async {
    final sw = Stopwatch()..start();
    final rows = await store.getHeaders(folderId: folderId, limit: 10000, offset: 0);
    ...
    final aggs = map.entries
        .map((e) => ThreadAggregate(key: e.key, messageUids: e.value..sort()))
        .toList();
