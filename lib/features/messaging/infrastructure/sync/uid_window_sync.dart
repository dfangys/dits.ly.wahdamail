...
class UidWindowSync {
  ...
  UidWindowSync({required this.gateway, required this.store, this.defaultWindow = 500});

  /// Compute next UID window(s) to fetch based on highest seen and remote max.
  Future<List<UidRange>> nextWindows({required String folderId, required int remoteMaxUid, int? windowSize, String? requestId}) async {
    ...
    while (start <= remoteMaxUid) {
      final end = (start + size - 1) <= remoteMaxUid ? (start + size - 1) : remoteMaxUid;
  ...
  Future<void> recordProgress({required String folderId, required int fetchedMaxUid}) async {
