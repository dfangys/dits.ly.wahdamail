...
class SendEmail {
  ...
  Future<OutboxItem> call({
    ...
    final queued = await outbox.enqueue(OutboxItem(
      id: draftId,
      accountId: accountId,
      folderId: folderId,
      messageId: messageId,
    ...
    try {
      ...
      return queued.copyWith(status: OutboxStatus.sent, updatedAt: DateTime.now());
    } catch (e) {
      final err = e is AppError ? e : mapSmtpError(e);
      final retryAt = retryPolicy.nextRetryAt(now: DateTime.now(), attemptCount: queued.attemptCount);
      await outbox.markFailed(id: queued.id, errorClass: err.runtimeType.toString(), retryAt: retryAt);
      return queued.copyWith(status: OutboxStatus.failed, lastErrorClass: err.runtimeType.toString(), retryAt: retryAt, updatedAt: DateTime.now());
    }
  }
}
