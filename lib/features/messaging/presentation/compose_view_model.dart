  ...
  Future<bool> send({
    ...
    if (!FeatureFlags.instance.dddKillSwitchEnabled &&
      ...
      try {
        ...
        final send = uc.SendEmail(drafts: drafts, outbox: outbox, smtp: smtp as dynamic);

        // Gather inputs
        final accountId = (GetStorage().read('email') as String?) ?? 'default-account';
        final folderId = (controller.sourceMailbox != null &&
        ...
        try {
          ...
          final mid = builtMessage.getHeaderValue('message-id') ?? builtMessage.getHeaderValue('Message-Id');
