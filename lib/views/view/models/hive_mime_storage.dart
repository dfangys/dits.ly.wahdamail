...
// Simple storage class for mailbox messages
class HiveMailboxMimeStorage {
  ...
  // ValueNotifier for UI updates - fixed initializer issue
  ...
  Future<void> init() async {
    ...
    try {
      ...
      // Initialize dataStream after _envelopeBox is created
  ...
  void _applyEnvelopeToMessage(MimeMessage message, Map envelope) {
    ...
    // Set seen flag - fixed by adding second parameter
    ...
    // Set date - fixed by using addHeader instead of setter
    if (envelope['date'] != null) {
      final timestamp = envelope['date'] as int;
      message.addHeader('Date', DateTime.fromMillisecondsSinceEpoch(timestamp).toIso8601String());
    }
  }
}

// These adapters are not needed with the simplified approach
class StorageMessageIdAdapter extends TypeAdapter<dynamic> {
  @override
  final int typeId = 1;
...
class StorageMessageEnvelopeAdapter extends TypeAdapter<dynamic> {
  @override
  final int typeId = 2;
