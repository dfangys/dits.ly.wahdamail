...
class ConnectionManager extends GetxService {
  ...
  static const int _fastReconnectThreshold = 3; // Fast reconnect for first 3 attempts
  ...
  Future<bool> connect({bool forceReconnect = false}) async {
    ...
    try {
      if (kDebugMode) {
        print('🔌 🚀 Attempting connection (attempt ${_consecutiveFailures + 1})');
      }
      ...
      await mailService.connect().timeout(
        _connectionTimeout,
        onTimeout: () => throw TimeoutException('Connection timeout', _connectionTimeout),
      );
  ...
  Future<void> _verifyConnection() async {
    ...
    try {
      ...
      if (mailService.client.mailboxes == null || mailService.client.mailboxes!.isEmpty) {
        await mailService.client.listMailboxes().timeout(const Duration(seconds: 10));
      }
  ...
  Future<void> _onConnectionFailure(dynamic error, Duration attemptDuration) async {
    ...
    if (_consecutiveFailures >= _maxConsecutiveFailures) {
      if (kDebugMode) {
        print('🔌 🚫 Max consecutive failures reached, stopping reconnection attempts');
      }
  ...
  void _updateConnectionQuality() {
    ...
    final successCount = _recentConnectionAttempts.where((success) => success).length;
    ...
    if (_connectionTimes.isNotEmpty) {
      final avgConnectionTime = _connectionTimes.fold<int>(0, (sum, time) => sum + time.inMilliseconds) / _connectionTimes.length;
      speedFactor = 1.0 - min(0.5, avgConnectionTime / 10000); // Penalize slow connections
    }
    
    // Factor in recent failures
    double stabilityFactor = 1.0;
    if (_consecutiveFailures > 0) {
      stabilityFactor = max(0.1, 1.0 - (_consecutiveFailures / _maxConsecutiveFailures));
    }
  ...
  void _trimHistory() {
    if (_connectionTimes.length > _maxHistorySize) {
      _connectionTimes.removeRange(0, _connectionTimes.length - _maxHistorySize);
    }
    if (_recentConnectionAttempts.length > _maxHistorySize) {
      _recentConnectionAttempts.removeRange(0, _recentConnectionAttempts.length - _maxHistorySize);
    }
  }
  ...
  void _performNetworkCheck() {
    ...
    } else if (_currentState == ConnectionState.disconnected && _consecutiveFailures < _maxConsecutiveFailures) {
  ...
  Map<String, dynamic> getConnectionStats() {
    final uptime = _lastSuccessfulConnection != null 
    ...
    return {
      ...
      'averageConnectionTime': _connectionTimes.isNotEmpty 
          ? '${(_connectionTimes.fold<int>(0, (sum, time) => sum + time.inMilliseconds) / _connectionTimes.length).round()}ms'
          : 'N/A',
      'successRate': _recentConnectionAttempts.isNotEmpty 
  ...
  bool get isHealthy => _currentState == ConnectionState.connected && _consecutiveFailures == 0;

  /// Check if should attempt reconnection
  bool get shouldReconnect => _currentState != ConnectionState.connected && 
