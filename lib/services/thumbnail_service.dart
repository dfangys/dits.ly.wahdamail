...
class ThumbnailService {
  ...
  Future<String> _generateCacheKey({
    ...
    try {
      ...
      final keyData =
          '${filePath}_${stat.size}_${stat.modified.millisecondsSinceEpoch}_${mimeType}_${maxWidth}x$maxHeight';
  ...
  Future<String?> getOrCreateThumbnail({
    ...
    try {
      ...
      if (generatedPath != null) {
        ...
        if (kDebugMode) {
          print(
            'ThumbnailService: Generated thumbnail for $filePath -> $generatedPath',
          );
        }
      }
  ...
  Future<String?> getOrCreateTextPreviewFromBytes({
    ...
    try {
      final key =
          'txtb|$idKey|${data.length}|${maxWidth}x$maxHeight|${mimeType.toLowerCase()}';
      final cacheKey = key.hashCode
          .abs()
          .toString()
          .padLeft(16, '0')
      ...
      final lines =
          text.split('\n').take(maxLines).map((l) {
            final s = l.trimRight();
            return s.length > maxCharsPerLine
                ? '${s.substring(0, maxCharsPerLine)}…'
                : s;
          }).toList();
  ...
  Future<void> _performCacheMaintenance() async {
    try {
      ...
      if (totalSize > _maxCacheSize || files.length > _maxCacheEntries) {
        ...
        for (final file in files) {
          if (currentSize <= _maxCacheSize * 0.8 &&
              currentCount <= _maxCacheEntries * 0.8) {
            break;
          }
          ...
          try {
            ...
            _memoryCache.removeWhere(
              (key, path) => p.basename(path) == fileName,
            );
          } catch (_) {}
        }
        ...
        if (kDebugMode) {
          print(
            'ThumbnailService: Cache maintenance completed. Size: ${currentSize ~/ 1024}KB, Count: $currentCount',
          );
        }
      }
      ...
      _cacheTimestamps.removeWhere(
        (key, time) => now.difference(time) > _cacheValidityDuration,
      );
