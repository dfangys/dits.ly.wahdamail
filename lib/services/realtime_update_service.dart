...
class RealtimeUpdateService extends GetxService {
  ...
  final BehaviorSubject<List<MimeMessage>> _messagesStream =
      BehaviorSubject<List<MimeMessage>>.seeded([]);
  final BehaviorSubject<Map<String, int>> _unreadCountsStream =
      BehaviorSubject<Map<String, int>>.seeded({});
  final BehaviorSubject<Set<String>> _flaggedMessagesStream =
      BehaviorSubject<Set<String>>.seeded({});
  final BehaviorSubject<ConnectionStatus> _connectionStatusStream =
      BehaviorSubject<ConnectionStatus>.seeded(ConnectionStatus.disconnected);
  final BehaviorSubject<SyncStatus> _syncStatusStream =
      BehaviorSubject<SyncStatus>.seeded(SyncStatus.idle);
  final BehaviorSubject<List<Mailbox>> _mailboxesStream =
      BehaviorSubject<List<Mailbox>>.seeded([]);

  // Message update streams for specific operations
  final PublishSubject<MessageUpdate> _messageUpdateStream =
      PublishSubject<MessageUpdate>();
  final PublishSubject<MailboxUpdate> _mailboxUpdateStream =
      PublishSubject<MailboxUpdate>();
  ...
  Stream<Set<String>> get flaggedMessagesStream =>
      _flaggedMessagesStream.stream;
      ...
      for (final mailbox in _mailboxesStream.value) {
        ...
        try {
          ...
          await c.reconcileRecentWithServer(
            mailbox,
            window: mailbox.isDrafts ? 1000 : 300,
          );
        } catch (_) {}
      }
  ...
  Future<void> _syncMailbox(Mailbox mailbox) async {
    try {
      ...
      if (mailService == null) {
        if (kDebugMode) {
          print(
            'MailService not available for syncing mailbox ${mailbox.name}',
          );
        }
        return;
      }
      ...
      await ImapCommandQueue.instance.run(
        'selectMailbox(syncMailbox:${mailbox.name})',
        () async {
    ...
    try {
      ...
      try {
        if (mailService.client.selectedMailbox?.encodedPath !=
            mailbox.encodedPath) {
          await ImapCommandQueue.instance.run(
            'selectMailbox(loadNewMessages:${mailbox.name})',
            () async {
      ...
      // Fetch new messages
      final newMessages = await ImapCommandQueue.instance.run(
        'fetchMessages(loadNewMessages:${mailbox.name})',
        () async {
  ...
  Future<void> _loadMailboxMessages(Mailbox mailbox) async {
    try {
      ...
      if (mailService == null) {
        _syncStatusStream.add(SyncStatus.error);
        _errorStream.add(
          'MailService not available for loading mailbox messages',
        );
        return;
      }
      ...
      await ImapCommandQueue.instance.run(
        'selectMailbox(loadMailboxMessages:${mailbox.name})',
        () async {
          await mailService.client.selectMailbox(mailbox);
        },
      );
      ...
      for (int i = 0; i < totalMessages; i += batchSize) {
        final end =
            (i + batchSize > totalMessages) ? totalMessages : i + batchSize;
        final count = end - i;
        final page = (i ~/ batchSize) + 1;

        final batchMessages = await ImapCommandQueue.instance.run(
          'fetchMessages(batch:${mailbox.name}:p$page:c$count)',
          () async {
            return await mailService.client.fetchMessages(
              mailbox: mailbox,
              count: count,
  ...
  Future<void> markMessageAsRead(MimeMessage message) async {
    try {
      ...
      if (mailService == null) {
        _errorStream.add(
          'MailService not available for marking message as read',
        );
      ...
      if (kDebugMode) {
        print(
          '📧 Marking message as read: UID=${message.uid}, SeqId=${message.sequenceId}',
        );
      }
      ...
      // Update streams
      _messageUpdateStream.add(
        MessageUpdate(message: message, type: MessageUpdateType.statusChanged),
      );

      // Refresh messages stream
      _messagesStream.add(
        _mailboxMessages.values.expand((msgs) => msgs).toList(),
      );
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> markMessageAsUnread(MimeMessage message) async {
    try {
      ...
      if (mailService == null) {
        _errorStream.add(
          'MailService not available for marking message as unread',
        );
        throw Exception('MailService not available');
      }
      ...
      if (kDebugMode) {
        print(
          '📧 Marking message as unread: UID=${message.uid}, SeqId=${message.sequenceId}',
        );
      }
  ...
  Future<void> deleteMessage(MimeMessage message) async {
    try {
      ...
      // Update streams
      _messageUpdateStream.add(
        MessageUpdate(message: message, type: MessageUpdateType.deleted),
      );
...
class MessageUpdate {
  ...
  MessageUpdate({required this.message, required this.type, this.metadata});
}
...
extension IncomingEmailExtension on RealtimeUpdateService {
  ...
  Future<void> notifyNewMessages(
    ...
    try {
      ...
      RealtimeUpdateService._logger.i(
        '📧 Processing ${newMessages.length} new messages',
      );

      // Determine target mailbox early for hydration fetches
      final String targetMailboxKey =
          (mailbox != null && mailbox.name.isNotEmpty)
              ? mailbox.name
              : ((mailbox != null && mailbox.encodedName.isNotEmpty)
                  ? mailbox.encodedName
                  : 'INBOX');
      ...
      try {
        ...
        for (final m in newMessages) {
          ...
          try {
            ...
            hasSubject =
                (s1 != null && s1.trim().isNotEmpty) ||
                ((m.envelope?.subject ?? '').trim().isNotEmpty);
          } catch (_) {
        ...
        if (toFetch.isNotEmpty) {
          ...
          if (mailService != null) {
            ...
            try {
              if (mailService.client.selectedMailbox?.encodedPath !=
                  targetMailbox.encodedPath) {
                await ImapCommandQueue.instance.run(
                  'selectMailbox(rt:env:${targetMailbox.name})',
                  () async {
                    await mailService.client
                        .selectMailbox(targetMailbox)
            ...
            for (final base in fetchList) {
              try {
                ...
                if (fetched.isNotEmpty) {
                  ...
                  if (env != null) {
                    base.envelope = env;
                    if ((base.from == null || base.from!.isEmpty) &&
                        (env.from?.isNotEmpty ?? false)) {
                      base.from = env.from;
                    }
                  }
                }
      ...
      for (final m in newMessages) {
        try {
          ...
          if (m.envelope == null &&
              ((m.from?.isNotEmpty ?? false) ||
                  (subj != null && subj.trim().isNotEmpty))) {
            try {
              m.envelope = Envelope(
              ...
              if (kDebugMode) {
                print(
                  '📧 RT fallback: synthesized minimal envelope for immediate tile hydration',
                );
              }
            } catch (_) {}
          }
      ...
      for (final message in newMessages) {
        ...
        int existingIndex = -1;
        try {
          existingIndex = listRef.indexWhere(
            (m) =>
        ...
        if (existingIndex >= 0) {
          ...
          try {
            ...
            if ((existing.from == null || existing.from!.isEmpty) &&
            ...
            if (pv != null && pv.trim().isNotEmpty)
              existing.setHeader('x-preview', pv);
            if ((message.getHeaderValue('x-ready') ?? '') == '1')
              existing.setHeader('x-ready', '1');
        ...
        if (!message.isSeen) {
          unreadCountChanges[mailboxKey] =
              (unreadCountChanges[mailboxKey] ?? 0) + 1;
        }
      ...
      for (final entry in unreadCountChanges.entries) {
        _unreadCounts[entry.key] =
            (_unreadCounts[entry.key] ?? 0) + entry.value;
      }
      ...
      RealtimeUpdateService._logger.i(
        '📧 Successfully processed ${newMessages.length} new messages',
      );
    } catch (e) {
