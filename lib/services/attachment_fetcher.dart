...
class AttachmentFetcher {
  ...
  static Future<Uint8List?> fetchBytes({
    ...
    try {
      if (kDebugMode) {
        print(
          'AttachmentFetcher: fetching ${content.fetchId} (${content.fileName ?? "unnamed"}) size: ${content.size ?? "unknown"} mime: ${content.contentType?.mediaType}',
        );
      }
      ...
      if (localData != null && localData.isNotEmpty) {
        if (_validateAttachmentData(localData, content)) {
          if (kDebugMode) {
            print(
              'AttachmentFetcher: local fetch successful (${localData.length} bytes)',
            );
          }
          return localData;
        }
      ...
      if (serverData != null && serverData.isNotEmpty) {
        if (_validateAttachmentData(serverData, content)) {
          if (kDebugMode) {
            print(
              'AttachmentFetcher: server fetch successful (${serverData.length} bytes)',
            );
          }
          return serverData;
        }
      ...
      if (kDebugMode) {
        print(
          'AttachmentFetcher: failed to fetch valid data for ${content.fetchId}',
        );
      }
      return null;
    } catch (e) {
  ...
  static Future<Uint8List?> _fetchFromLocalPart(
    MimeMessage message,
    ContentInfo content,
  ) async {
    try {
        ...
        if (_looksLikeBase64(textData)) {
          try {
            return Uint8List.fromList(
              base64.decode(textData.replaceAll(RegExp(r'\s'), '')),
            );
          } catch (_) {
    ...
    while (retryCount < _maxRetries) {
      try {
        ...
        if (mailbox != null) {
          ...
          if (selected?.encodedPath != mailbox.encodedPath &&
              selected?.path != mailbox.path) {
            await mail.client
        ...
        if (kDebugMode) {
          print(
            'AttachmentFetcher: server fetch attempt $retryCount failed: $e',
          );
        }
        ...
        if (retryCount >= _maxRetries) {
          if (kDebugMode) {
            print(
              'AttachmentFetcher: server fetch failed after $retryCount attempts',
            );
          }
          return null;
        }
  ...
  static bool _validateAttachmentData(Uint8List data, ContentInfo content) {
    ...
    if (declaredSize != null && declaredSize > 0) {
      ...
      if (sizeDiff > declaredSize * 0.1 && sizeDiff > 1024) {
        if (kDebugMode) {
          print(
            'AttachmentFetcher: size mismatch - expected: $declaredSize, actual: ${data.length}',
          );
        }
    ...
    if (mimeType != null) {
      if (!_validateContentByMimeType(data, mimeType)) {
        if (kDebugMode) {
          print(
            'AttachmentFetcher: content validation failed for MIME type: $mimeType',
          );
        }
        return false;
      }
  ...
  static Future<Uint8List?> fetchByFetchId({
    ...
    try {
      ...
      if (part != null) {
        ...
        final data =
            part.mimeData?.decodeBinary(enc) ?? part.decodeContentBinary();
