...
class AttachmentValidator {
  ...
  static const Set<String> _dangerousExtensions = {
    ...
    'deb', 'pkg', 'rpm', 'dmg', 'iso', 'msi', 'reg', 'ps1', 'sh', 'bin'
  };
  ...
  Future<ValidationResult> validateBytes({
    ...
    try {
      ...
      if (bytes.length > _maxFileSize) {
        return ValidationResult(
          isValid: false,
          error: 'File size ${bytes.length} exceeds maximum allowed size $_maxFileSize',
          detectedMimeType: declaredMimeType ?? 'application/octet-stream',
      ...
      if (expectedSize != null && bytes.length != expectedSize) {
        return ValidationResult(
          isValid: false,
          error: 'File size mismatch: expected $expectedSize, got ${bytes.length}',
          detectedMimeType: declaredMimeType ?? 'application/octet-stream',
          sanitizedFilename: sanitizedFilename,
        );
      }
      ...
      if (filename.length > _maxFilenameLength) {
        return ValidationResult(
          isValid: false,
          error: 'Filename exceeds maximum length of $_maxFilenameLength characters',
          detectedMimeType: declaredMimeType ?? 'application/octet-stream',
          sanitizedFilename: sanitizedFilename,
        );
      }
      ...
      // 6. Security checks
      final securityResult = _performSecurityChecks(bytes, filename, detectedMimeType);
      ...
      return ValidationResult(
        ...
        metadata: {...metadata, ...contentResult.metadata, ...securityResult.metadata},
      );
  ...
  ValidationResult _validateMimeType({
    ...
    if (declaredMimeType != detectedMimeType) {
      ...
      for (final entry in allowedMismatches.entries) {
        if ((entry.key == declaredMimeType && entry.value.contains(detectedMimeType)) ||
            (entry.key == detectedMimeType && entry.value.contains(declaredMimeType))) {
      ...
      if (!isAllowedMismatch) {
        return ValidationResult(
          ...
          warning: 'MIME type mismatch: declared "$declaredMimeType" but detected "$detectedMimeType"',
          detectedMimeType: detectedMimeType,
          sanitizedFilename: _sanitizeFilename(filename),
          isSuspicious: true,
        );
  ...
  Future<ValidationResult> _validateContent(Uint8List bytes, String mimeType) async {
    final metadata = <String, dynamic>{};
  ...
  Future<ValidationResult> _validateTextContent(Uint8List bytes, Map<String, dynamic> metadata) async {
    try {
  ...
  Future<ValidationResult> _validateImageContent(Uint8List bytes, String mimeType, Map<String, dynamic> metadata) async {
  ...
  Future<ValidationResult> _validateJsonContent(Uint8List bytes, Map<String, dynamic> metadata) async {
    try {
  ...
  ValidationResult _performSecurityChecks(Uint8List bytes, String filename, String mimeType) {
    final metadata = <String, dynamic>{};
    final warnings = <String>[];
    bool isSuspicious = false;
    
    // Check for embedded executables in non-executable files
    if (!mimeType.contains('application/') || !filename.toLowerCase().contains('.exe')) {
    ...
    final suspiciousPatterns = [
      'eval(', 'javascript:', '<script', 'powershell', 'cmd.exe',
      'base64', 'eval', 'unescape', 'fromCharCode'
    ];
    
    if (bytes.length < 10000) { // Only check smaller files to avoid performance issues
      try {
  ...
  String _sanitizeFilename(String filename) {
    ...
    if (sanitized.length > _maxFilenameLength) {
      final extension = sanitized.contains('.') 
          ...
          : '';
      sanitized = sanitized.substring(0, _maxFilenameLength - extension.length) + extension;
    }
