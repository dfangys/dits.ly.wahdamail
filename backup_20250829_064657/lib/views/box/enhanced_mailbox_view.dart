...
class _EnhancedMailboxViewState extends State<EnhancedMailboxView>
  ...
  /// Build initialization loading screen (enterprise-grade shimmer list)
  Widget _buildInitializationLoading() {
    // Show a list skeleton shimmer while the mailbox initializes
  ...
  /// Build main email list with shimmer placeholders for unready messages
  ...
  Widget _buildEmailList(List<MimeMessage> all) {
    // Include ALL messages; unready ones will render with shimmer until ready
    final allEmails = all.toList(growable: false)
      ..sort((a, b) {
    ...
    // If still no emails loaded, keep the empty state (shimmer shown during init)
    if (allEmails.isEmpty) {
      return _buildEmptyState();
    }

    // Virtualization tuning & accessibility
    final textScale = MediaQuery.of(context).textScaler.scale(14.0) / 14.0;
    final useFixedExtent = FeatureFlags.instance.virtualizationTuningEnabled && FeatureFlags.instance.fixedExtentListEnabled && textScale <= 1.3;
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return RepaintBoundary(child: _buildMaybeShimmerTile(message));
        },
      ),
    );
  }

  Widget _buildMaybeShimmerTile(MimeMessage message) {
