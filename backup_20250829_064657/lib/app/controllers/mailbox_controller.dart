    ...
    try {
      ...
      _mailboxUpdateSubscription = realtimeService.mailboxUpdateStream.listen((update) {
        try {
          ...
          if ((update.type == MailboxUpdateType.messagesAdded || update.type == MailboxUpdateType.newMessages) && update.messages != null) {
...
logger.i("Loading messages $sequenceStart-$sequenceEnd for page $pageNumber");
    ...
    try {
      ...
      if (mailbox != null && mailboxStorage[mailbox] != null) {
        ...
        try {
          ...
          await realtimeService.notifyNewMessages([message], mailbox: mailbox);
  ...
  /// Open specific email by UID from a mailbox path (used by notification tap)
  Future<void> openEmailByUid(String mailboxPath, int uid) async {
    try {
      ...
      // Find mailbox
      Mailbox? mailbox = mailboxes.firstWhereOrNull((m) =>
          m.path == mailboxPath || m.encodedPath == mailboxPath || m.name == mailboxPath);
      if (mailbox == null) {
        try {
          final listed = await mailService.client.listMailboxes();
          mailboxes(listed);
          mailbox = listed.firstWhereOrNull((m) =>
              m.path == mailboxPath || m.encodedPath == mailboxPath || m.name == mailboxPath);
        } catch (_) {}
      }
      ...
      MimeMessage? msg = emails[mailbox]?.firstWhereOrNull((m) => m.uid == uid);

      // If not found, fetch by UID
      if (msg == null) {
        try {
          ...
          if (fetched.isNotEmpty) {
            ...
            final list = emails[mailbox] ?? <MimeMessage>[];
            final exists = list.any((m) => m.uid == msg!.uid);
            if (!exists) {
              list.insert(0, msg!);
              emails[mailbox] = list;
              emails.refresh();
      ...
      if (msg != null) {
        await safeNavigateToMessage(msg, mailbox);
      } else {
        // As a fallback, navigate to mailbox view
        await navigatToMailBox(mailbox);
      }
    } catch (e) {
      logger.w('openEmailByUid error: $e');
    }
  }
      ...
      Future<void> worker() async {
        while (true) {
          ...
          try {
            ...
            // Replace in-memory message with full version (for future accesses)
            try {
            ...
            // Stamp headers for immediate UI benefit on BOTH full and base instances
            try {
              if (preview.isNotEmpty) {
                try { full.setHeader('x-preview', preview); } catch (_) {}
                try { base.setHeader('x-preview', preview); } catch (_) {}
              }
              try { full.setHeader('x-has-attachments', hasAtt ? '1' : '0'); } catch (_) {}
              try { base.setHeader('x-has-attachments', hasAtt ? '1' : '0'); } catch (_) {}
              ...
              try {
                ...
                try { full.setHeader('x-thread-count', '${tc <= 0 ? 1 : tc}'); } catch (_) {}
                try { base.setHeader('x-thread-count', '${tc <= 0 ? 1 : tc}'); } catch (_) {}
              } catch (_) {}
              try { full.setHeader('x-ready', '1'); } catch (_) {}
              try { base.setHeader('x-ready', '1'); } catch (_) {}
              // Notify tile listeners for both instances
              try { bumpMessageMeta(mailbox, full); } catch (_) {}
              try { bumpMessageMeta(mailbox, base); } catch (_) {}
            } catch (_) {}
      ...
      Future<void> worker() async {
        while (true) {
          ...
          try {
            ...
            // Stamp headers on BOTH full and base to flip shimmer in-place
            try {
              if (preview.isNotEmpty) {
                try { full.setHeader('x-preview', preview); } catch (_) {}
                try { base.setHeader('x-preview', preview); } catch (_) {}
