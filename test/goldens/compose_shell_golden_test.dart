...
void main() {
  ...
  testWidgets('Compose shell golden - dark', (tester) async {
    tester.binding.window.devicePixelRatioTestValue = 1.0;
    tester.binding.window.physicalSizeTestValue = const Size(800, 1200);

    await tester.pumpWidget(_composeShellHarness(AppThemeDS.dark));
    await tester.pumpAndSettle();

    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('goldens/compose_shell_dark.png'),
    );
  });

  testWidgets('Compose shell golden - light @1.3x', (tester) async {
    tester.binding.window.devicePixelRatioTestValue = 1.0;
    tester.binding.window.physicalSizeTestValue = const Size(800, 1200);
    ...
    await tester.pumpWidget(
      MediaQuery(
        ...
        child: _composeShellHarness(AppThemeDS.light),
      ),
    );
    await tester.pumpAndSettle();
    ...
    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('goldens/compose_shell_1_3x.png'),
    );
  });
}
