...
class _FakeGateway implements ImapGateway {
  ...
  }) async => BodyDTO(messageUid: messageUid, mimeType: 'text/plain');
...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(
      const entities.Folder(id: 'INBOX', name: 'INBOX', isInbox: true),
    );
  });
  ...
  test('SyncService triggers header fetch on IDLE events', () async {
    ...
    verify(
      () => repo.fetchInbox(
        folder: const entities.Folder(id: 'INBOX', name: 'INBOX'),
        limit: 50,
        offset: 0,
      ),
    ).called(1);
  ...
  test('SyncService coalesces burst events within debounce window', () async {
    ...
    gw.ctrl.add(const ImapEvent(type: ImapEventType.exists, folderId: 'INBOX'));
    gw.ctrl.add(
      const ImapEvent(type: ImapEventType.flagsChanged, folderId: 'INBOX'),
    );
    gw.ctrl.add(
      const ImapEvent(type: ImapEventType.expunge, folderId: 'INBOX'),
    );
