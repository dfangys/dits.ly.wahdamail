...
void main() {
  group('Local search via repository', () {
    test(
      'filters by subject/from/to/date and includes body when cached; sorts and limits',
      () async {
        ...
        await store.upsertHeaders([
          ...
          MessageRow(
            ...
            toEmails: const ['alice@example.com'],
            dateEpochMs: 2000,
            seen: false,
            answered: false,
            flagged: true,
        ...
        await store.upsertBody(
          BodyRow(
            messageUid: '1',
            mimeType: 'text/plain',
            plainText: 'meet at 5pm',
          ),
        );

        // Subject filter
        var res = await repo.search(
          accountId: 'acct',
          q: dom.SearchQuery(subject: 'plans'),
        );
        ...
        res = await repo.search(
          accountId: 'acct',
          q: dom.SearchQuery(flags: {'flagged'}),
        );
        ...
        res = await repo.search(
          accountId: 'acct',
          q: dom.SearchQuery(
            dateFrom: DateTime.fromMillisecondsSinceEpoch(1500),
          ),
        );
        expect(res.map((e) => e.messageId), ['2']);
