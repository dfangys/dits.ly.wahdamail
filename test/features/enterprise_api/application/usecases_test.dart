...
void main() {
  ...
  test('FetchAccountProfile orchestrates repo call', () async {
    ...
    final uid = UserId('u1');
    when(() => repo.fetchAccountProfile(userId: uid)).thenAnswer(
      (_) async => AccountProfile(userId: uid, email: 'e', displayName: 'n'),
    );
  ...
  test('ListContacts orchestrates repo call', () async {
    ...
    ).thenAnswer((_) async => [Contact(id: 'c1', name: 'n', email: 'e')]);
  ...
  test('UpsertSignature orchestrates repo call', () async {
    ...
    when(
      () => repo.upsertSignature(userId: uid, signature: sig),
