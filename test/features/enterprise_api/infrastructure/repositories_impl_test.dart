...
class _FakeGateway extends RestGateway {
  ...
  @override
  Future<AccountDto> fetchAccountProfile({
  ...
  @override
  Future<SignatureDto> upsertSignature({
    required String userId,
    required String accessToken,
    required SignatureDto dto,
  }) async {
    signatureCalls++;
    return SignatureDto(
      id: dto.id,
...
void main() {
  test(
    'AccountsRepositoryImpl.refreshToken updates store and returns new token',
    () async {
  ...
  test(
    'AccountsRepositoryImpl.fetchAccountProfile retries after refresh on AuthError',
    () async {
      final gw = _FakeGateway();
      final store = InMemoryTokenStore();
      final repo = AccountsRepositoryImpl(gateway: gw, tokens: store);
  ...
  test('SignaturesRepositoryImpl.upsertSignature happy path', () async {
    ...
    final sig = await repo.upsertSignature(
      userId: uid,
