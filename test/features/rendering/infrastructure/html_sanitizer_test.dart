...
void main() {
  test(
    'Sanitizer removes script/iframe/on* and blocks remote images by default',
    () {
      final s = HtmlSanitizer();
      final html =
          '<div onload="alert(1)"><script>alert(1)</script><iframe src="x"></iframe><img src="http://t"/><a href="javascript:evil()">x</a></div>';
  ...
  test(
    'Sanitizer allows remote images when allowRemote=true and flags hasRemoteAssets',
    () {
      final s = HtmlSanitizer();
  ...
  test('Sanitizer strips data: images, srcset, style url() and formaction', () {
    final s = HtmlSanitizer();
    final html =
        '<img src="data:image/png;base64,AAAA" srcset="http://a 1x" style="background:url(http://x)" formaction="http://evil">';
    final res = s.sanitize(html, allowRemote: true);
